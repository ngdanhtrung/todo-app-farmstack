...
function App() {
  ...
  useEffect(() => {
    ...
    const timelineControlsItem = todoList.map((task) => {
      return <TodoItem todo={task} getTodoList={getTodoList} />;
    });
    setTimelineControls(timelineControlsItem);
    ...
    >
      ...
      <div className='mt-5'>
        <Row>
          ...
          <Col span={12}>{timelineControls}</Col>
        </Row>
      </div>
    </div>
  );
